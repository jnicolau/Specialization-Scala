...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("No element in Empty TweetSet.")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweetAcc(acc: TweetList): TweetList = {
    ...
    new Cons(mostRetweeted, remove(mostRet).descendingByRetweetAcc(acc))
  }
...
object GoogleVsApple {
  ...
  def exists(keywords: List[String], text: String): Boolean = keywords.exists(k => text.contains(k))
  lazy val googleTweets: TweetSet = allTweets.filter(tweet => exists(google, tweet.text))
  lazy val appleTweets: TweetSet = allTweets.filter(tweet => exists(apple, tweet.text))
