...
class ReductionsSuite {
  ...
  @Test def `upsweepSequential should correctly handle the chunk 1 until 4 of an array of 4 elements`: Unit = {
  ...
  @Test def `parLineOfSight should correctly handle an array of size 4 and threshold 4`: Unit = {
    val output = new Array[Float](4)
    parLineOfSight(Array[Float](0f, 1f, 8f, 9f), output, 4)
    assert(output.toList == List(0f, 1f, 4f, 4f))
  }

  @Test def `parLineOfSight should correctly handle an array of size 4 and threshold 1`: Unit = {
    val output = new Array[Float](4)
